 ...
 Class News extends Common
 {
 	...
 	public function index(){
		...
		$adpage =$this->adpage;//分页时每页的条数
 		...
 		$list = Db::name('news')->where("status",0)->order('sort desc')->paginate($adpage)->each(function($item, $key){
 			$id = $item['catid'];
 	...
 	public function add(){
 		...
 		if(empty($data)){
			...
			foreach($result as $k=>$v){
 			...
 			$this->assign('result',$result);
  			return $this->fetch();			
 				...
 				*/
 				if($data['title']==''){
 					$this->error('标题不能为空','news/add');
 				}
 				if($data['content']==''){
 					$this->error('文章内容不能为空','news/add');
 				}

 			$file = request()->file('thumb');//获取上传的文件
 			$riqi = date("Ymd",time());
 			if($file){
				$info =$file->move(ROOT_PATH.'public'.DS.'uploads'.DS.'image');//将上传的文件移到指定文件夹
				$img =(ROOT_PATH.'public'.DS.'uploads'.DS.'image'.DS.$riqi.DS.$info->getFilename());//获取上传文件的完整路径
				$data['thumb'] = $img;
			}else{
				$data['thumb'] = '';
			}
			...
			*/
			...
			if($getnum>0){
				$this->success('文章添加成功','news/index');
			}else{
				$this->error('文章添加失败');
			}	
 	...
 	public function save(){
 		...
 		$riqi = date("Ymd",time());
			...
			*/
			...
			$getnum = Db::name('news')->update($data);
			if($getnum>0){
				$this->success('文章修改成功');
			}else{
				$this->error('文章修改失败');
 	...
 	**/
 	public function setchange(){
		...
		if($shome==1){
			...
			$list = Db::name('news')->where('posids',1)->whereor('posids',3)->whereor('posids',5)->whereor('posids',7)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
		...
		//推荐位不是首页		
			...
			$list = Db::name('news')->where('posids',0)->whereor('posids',2)->whereor('posids',4)->whereor('posids',6)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
		...
		//推荐位是栏目
			...
			$list = Db::name('news')->where('posids',2)->whereor('posids',3)->whereor('posids',6)->whereor('posids',7)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
		...
		//推荐位不是栏目
			...
			$list = Db::name('news')->where('posids',0)->whereor('posids',1)->whereor('posids',4)->whereor('posids',5)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
		...
		//推荐位是其他
			...
			$list = Db::name('news')->where('posids',4)->whereor('posids',5)->whereor('posids',6)->whereor('posids',7)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
		...
		//推荐位不是其他	
			...
			$list = Db::name('news')->where('posids',0)->whereor('posids',1)->whereor('posids',2)->whereor('posids',3)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
		...
		//栏目id有值时
			...
			$list = Db::name('news')->where('catid',$cid)->order('sort desc')->paginate(5,false,['query' =>['shome'=>$shome] ])->each(function($item, $key){
 			$id = $item['catid'];
 			$column_id = Db::name('column')->where('id',$id)->find();
 			$item['catid']=$column_id['catname'];
 			$item['addtime'] = date('Y-m-d H:s:i',$item['addtime']);
